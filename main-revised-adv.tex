...
use is not new.  For the nonlazy complete-graph walk,
Gu--Polyanskiy's one-parameter reduction and strictness argument
\cite[equation~(30) and the proof of Proposition~33]{GuPolyanskiy2023}
...
\[
  \Mone=\frac{\log n-\log(n-1)}{\log n},
  \qquad \Lone=1.
\]
In this example the lower bound is exact:
\[
  \etaStat=\Mone,
\]
and point masses are the only extremizers
\cite[equation~(30) and the proof of Proposition~33]{GuPolyanskiy2023}.
This is a useful benchmark precisely
because it is analytic; no optimizer output is needed.
...
\begin{figure}[H]
  ...
  under each profile is \(\Lone\).  The lazy cycle is homogeneous
  (\(\Lone=1\)), while the lazy star has a two-level profile
  (\(\Lone=1/2\)).  The third panel
  uses the finite multiplicity construction with
  \(\varepsilon=0.08\), \(k=256\), \(N=300\), and \(\delta=0.005\):
  \(\Lone=0.122\) but the set witness has \(R(A)=0.983\).
  The small singleton area therefore coexists with almost complete
...
\path{scripts/generate_retention_profile_figure.py}; it uses only the
closed-form expressions displayed in the paper.  The companion script
\path{scripts/validate_retention_claims.py} independently checks the finite
counterexample matrix, the diagonal construction, the three-state spectra
and profiles, the binary comparison, and the simplified bottleneck
inequality.  No empirical dataset is used.  Both scripts are included in
the submission source archive.  The manuscript makes no optimizer-based
claims.
